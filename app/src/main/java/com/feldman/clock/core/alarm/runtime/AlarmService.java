...
public class AlarmService extends Service {
     ...
     * ALARM_DONE_ACTION).
     */
    public static final String ALARM_SNOOZE_ACTION = "com.feldman.clock.ALARM_SNOOZE";
     ...
     */
    public static final String ALARM_DISMISS_ACTION = "com.feldman.clock.ALARM_DISMISS";

    /**
     * A public action sent by AlarmService when the alarm has started.
     */
    public static final String ALARM_ALERT_ACTION = "com.feldman.clock.ALARM_ALERT";

    /**
     * A public action sent by AlarmService when the alarm has stopped for any reason.
     */
    public static final String ALARM_DONE_ACTION = "com.feldman.clock.ALARM_DONE";
    ...
    private final BroadcastReceiver mActionsReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i("AlarmService received intent %s", action);
            if (mCurrentAlarm == null || mCurrentAlarm.mAlarmState != AlarmInstance.FIRED_STATE) {
                Log.i("AlarmService", "No valid firing alarm");
                return;
            }

            if (mIsBound) {
                Log.i("AlarmService", "AlarmActivity bound; AlarmService no-op");
                return;
            }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent == null) {
            com.feldman.clock.core.util.FileLog.e("AlarmService", "onStartCommand received null intent");
            return Service.START_NOT_STICKY;
        }

        Log.v("AlarmService.onStartCommand() with %s", intent.toString());
        com.feldman.clock.core.util.FileLog.i("AlarmService", "onStartCommand: " + intent);
        ...
        try {
            switch (Objects.requireNonNull(intent.getAction())) {
                case AlarmStateManager.CHANGE_STATE_ACTION -> {
                    Log.i("AlarmService", "onStartCommand received CHANGE_STATE_ACTION: " + intent);
                    ...
                    if (alarmState == AlarmInstance.FIRED_STATE) {
                        ...
                        if (instance == null) {
                            Log.e("AlarmService", "No instance found to start alarm: " + instanceId);
                            com.feldman.clock.core.util.FileLog.e("AlarmService", "No instance found to start alarm: " + instanceId);
                        ...
                        if (mCurrentAlarm != null && mCurrentAlarm.mId == instanceId) {
                            Log.e("AlarmService", "Alarm already started for instance: %d");
                            break;
                        }
                        startAlarm(instance);
                    }
                ...
                case STOP_ALARM_ACTION -> {
                    if (mCurrentAlarm != null && mCurrentAlarm.mId != instanceId) {
                        Log.e("AlarmService", "Can't stop alarm for instance: %d because current alarm is: %d");
                        break;
                    }
                    stopCurrentAlarm();
                    stopSelf();
                ...
                case STOP_ALARM_WITH_DOUBLE_VIBRATION_ACTION -> {
                    if (mCurrentAlarm != null && mCurrentAlarm.mId != instanceId) {
                        Log.e("AlarmService", "Can't perform double vibration and stop alarm for instance: %d ");
                        break;
                    }
                ...
                case STOP_ALARM_WITH_SINGLE_VIBRATION_ACTION -> {
                    if (mCurrentAlarm != null && mCurrentAlarm.mId != instanceId) {
                        Log.e("AlarmService", "Can't perform single vibration and stop alarm for instance: %d " +
                                        "because current alarm is: %d");
                        break;
                    }
            ...
            com.feldman.clock.core.util.FileLog.e("AlarmService", "Error in onStartCommand", e);
            stopSelf();
        }
    ...
    private void startAlarm(AlarmInstance instance) {
        Log.v("AlarmService", "AlarmService.start with instance: " + instance.mId);
        if (mCurrentAlarm != null) {
        ...
        try {
            AlarmNotifications.showAlarmNotification(this, mCurrentAlarm);
             ...
             Log.e("AlarmService", "Failed to show alarm notification", e);
             com.feldman.clock.core.util.FileLog.e("AlarmService", "Failed to show alarm notification", e);
             // Keep going, maybe Activity will show up
        }

        try {
            AlarmRinger.start(this, mPrefs, mCurrentAlarm);
        } catch (Exception e) {
             Log.e("AlarmService", "Failed to start ringer", e);
             com.feldman.clock.core.util.FileLog.e("AlarmService", "Failed to start ringer", e);
        }

        boolean globalFlash = SettingsDAO.shouldTurnOnBackFlashForTriggeredAlarm(mPrefs);
        // The global setting is the master switch: it enables the feature and seeds newly created
        // alarms. When it is on, each alarm's own "Flashlight" toggle decides whether it pulses.
        if (globalFlash && mCurrentAlarm.mFlash) {
            mHandler.post(mFlashRunnable);
        }
        sendBroadcast(new Intent(ALARM_ALERT_ACTION));
        
        // Refresh settings before attaching listeners to ensure we use the latest user preferences
        mFlipAction = SettingsDAO.getFlipAction(mPrefs);
        mShakeAction = SettingsDAO.getShakeAction(mPrefs);
        Log.i("AlarmService", "Refreshed sensor actions: flip=" + mFlipAction + ", shake=" + mShakeAction);
    ...
    private void stopCurrentAlarm() {
        ...
        Log.v("AlarmService.stop with instance: %s", String.valueOf(instanceId));

        AlarmRinger.stop(this, mPrefs);

        AlarmRinger.deactivateRingtonePlayback(mPrefs);
    ...
    private void performSingleVibration() {
        if (mCurrentAlarm == null) {
            Log.v("AlarmService", "There is no current alarm to stop so it's impossible to perform a single vibration");
            return;
        }

        final long instanceId = mCurrentAlarm.mId;
        Log.v("AlarmService.stop with single vibration with instance: %s", String.valueOf(instanceId));
    ...
    private void performDoubleVibration() {
        if (mCurrentAlarm == null) {
            Log.v("AlarmService", "There is no current alarm to stop so it's impossible to perform a double vibration");
            return;
        }

        final long instanceId = mCurrentAlarm.mId;
        Log.v("AlarmService.stop with double vibration with instance: %s", String.valueOf(instanceId));
    ...
    private void getBackCameraId() {
        try {
            ...
            if (mCameraId == null) {
                Log.e("AlarmService", "mCameraId is null");
            }
        } catch (CameraAccessException e) {
            Log.e("AlarmService", "AlarmService.onCreate - Failed to access the flash unit", e);
        }
    }
    ...
    private void toggleFlash(boolean state) {
        try {
            ...
            Log.e("AlarmService", "AlarmService.toggleFlash - Failed to access the flash unit", e);
        }
    }
    ...
    private void attachListeners() {
        Log.i("AlarmService", "Attaching sensor listeners. Flip: " + mFlipAction + ", Shake: " + mShakeAction);
        if (mFlipAction != ALARM_NO_ACTION) {
            ...
            Log.i("AlarmService", "Flip listener registered");
        }
        ...
        if (mShakeAction != ALARM_NO_ACTION) {
            ...
            Log.i("AlarmService", "Shake listener registered");
        }
    }
    ...
    private void handleAction(int action) {
        if (action == ALARM_SNOOZE) { // Setup Snooze Action
            Log.i("AlarmService", "Handling action: SNOOZE (" + action + ")");
            ...
            Log.i("AlarmService", "Handling action: DISMISS (" + action + ")");
            startService(AlarmStateManager.createStateChangeIntent(this,
                    AlarmStateManager.ALARM_DISMISS_TAG, mCurrentAlarm, AlarmInstance.DISMISSED_STATE));
        } else {
            Log.i("AlarmService", "Handling unknown action: " + action);
        }
    }
