...
import static com.feldman.clock.settings.PreferencesDefaultValues.ALARM_SNOOZE_DURATION_DISABLED;
import static com.feldman.clock.core.util.AlarmUtils.ACTION_NEXT_ALARM_CHANGED_BY_CLOCK;
...
public final class AlarmStateManager extends BroadcastReceiver {
     ...
     * <ul>
     *     <li>in the Clock app (alarm tab and standby)</li>
    ...
    private static void updateNextAlarmInAlarmManager(Context context, AlarmInstance nextAlarm) {
        ...
        if (nextAlarm != null) {
            Log.i("AlarmService", "Setting upcoming AlarmClockInfo for alarm: " + nextAlarm.mId);
            ...
            Log.i("AlarmService", "Canceling upcoming AlarmClockInfo");
    ...
    private static void updateParentAlarm(Context context, AlarmInstance instance) {
        ...
        if (alarm == null) {
            Log.e("AlarmService", "Parent has been deleted with instance: " + instance);
            return;
        }

        if (!alarm.daysOfWeek.isRepeating()) {
            if (alarm.deleteAfterUse) {
                Log.i("AlarmService", "Deleting parent alarm: " + alarm.id);
                ...
                Log.i("AlarmService", "Disabling parent alarm: " + alarm.id);
                alarm.enabled = false;
            ...
            Log.i("AlarmService", "Creating new instance for repeating alarm " + alarm.id + " at " +
    ...
    public static Intent createStateChangeIntent(Context context, String tag,
        ...
        // The reason is that evidence exists showing the jump between the
        ...
        // UPDATE: Targeting AlarmService directly fails on Android 10+ if app is in background.
        // We now target AlarmReceiver which acquires a WakeLock and starts the service in foreground.
        Intent intent = AlarmInstance.createIntent(context, AlarmReceiver.class, instance.mId);
    ...
    public static void setSilentState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting silent state to instance " + instance.mId);
    ...
    public static void setNotificationState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting notification state to instance " + instance.mId);
    ...
    public static void setFiredState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting fire state to instance " + instance.mId);
    ...
    public static void setSnoozeState(final Context context, AlarmInstance instance, boolean showToast) {
        ...
        Log.i("AlarmService", "Setting snoozed state to instance " + instance.mId + " for "
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting missed state to instance " + instance.mId);
        ...
        Log.i("AlarmService", "Marking instance as MISSED: " + instance.mId);
    ...
    public static void setPreDismissState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting predismissed state to instance " + instance.mId);
    ...
    public static void setDismissState(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Setting dismissed state to instance " + instance.mId);
        instance.mMissedAlarmCurrentCount = 0;
    ...
    public static void deleteInstanceAndUpdateParent(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Deleting instance " + instance.mId + " and updating parent alarm.");
    ...
    public static void unregisterInstance(Context context, AlarmInstance instance) {
        Log.i("AlarmService", "Unregistering instance " + instance.mId);
        AlarmNotifications.clearNotification(context, instance);
    ...
    public static void registerInstance(Context context, AlarmInstance instance, boolean updateNextAlarm) {
        Log.i("AlarmService", "Registering instance: " + instance.mId);
        final ContentResolver cr = context.getContentResolver();
        ...
        if (instance.mAlarmState == AlarmInstance.DISMISSED_STATE) {
            ...
            Log.e("AlarmService", "Alarm Instance is dismissed, but never deleted");
            deleteInstanceAndUpdateParent(context, instance);
            return;
            ...
            if (currentTime.before(alarmTime)) {
                if (instance.mAlarmId == null) {
                    Log.i("AlarmService", "Cannot restore missed instance for one-time alarm");
        ...
        if (currentTime.after(missedTTL)) {
            ...
            com.feldman.clock.core.util.FileLog.i("AlarmStateManager", "Notifications: Current time " + currentTime.getTime() + " is after notification time " + notificationTime.getTime() + ". Showing notification.");
            setNotificationState(context, instance);
        } else {
            com.feldman.clock.core.util.FileLog.i("AlarmStateManager", "Notifications: Current time " + currentTime.getTime() + " is before notification time " + notificationTime.getTime() + ". Scheduling silent state.");
    ...
    public static void deleteAllInstances(Context context, long alarmId) {
        Log.i("AlarmService", "Deleting all instances of alarm: " + alarmId);
        ContentResolver cr = context.getContentResolver();
    ...
    public static void fixAlarmInstances(Context context) {
        Log.i("AlarmService", "Fixing alarm instances");
        ...
        for (AlarmInstance instance : instances) {
            ...
            if (alarm == null) {
                ...
                Log.e("AlarmService", "Found instance without matching alarm; deleting instance %s");
                continue;
            }
            ...
            if (currentTime.before(priorAlarmTime) || currentTime.after(missedTTLTime)) {
                ...
                Log.i("AlarmService", "A time change has caused an existing alarm scheduled to fire at %s to" +
    ...
    private static void setAlarmState(Context context, AlarmInstance instance, int state) {
        if (instance == null) {
            Log.e("AlarmService", "Null alarm instance while setting state to %d");
            return;
        }
        switch (state) {
            ...
            default -> Log.e("AlarmService", "Trying to change to unknown alarm state: " + state);
        }
    }

    public static void handleIntent(Context context, Intent intent) {
        final String action = intent.getAction();
        Log.v("AlarmService", "AlarmStateManager received intent " + intent);
        if (CHANGE_STATE_ACTION.equals(action)) {
            ...
            if (instance == null) {
                Log.e("AlarmService", "Can not change state for unknown instance: " + uri);
                return;
            }
            ...
            if (instance == null) {
                Log.e("AlarmService", "Null AlarmInstance for SHOW_AND_DISMISS");
            ...
            final Intent viewAlarmIntent = Alarm.createIntent(context, MainActivity.class, alarmId)
    ...
    private static void setPowerOffAlarm(Context context, AlarmInstance instance) {
        ...
        try {
            Log.i("AlarmService", "Set next power off alarm : instance id " + instance.mId);
            ...
            Log.e("AlarmService", "Failed to send power off alarm broadcast: " + e);
        }
    }
    ...
    private static void cancelPowerOffAlarm(Context context, AlarmInstance instance) {
        if (DeviceUtils.isPowerOffAlarmUnSupported()) {
            Log.w("AlarmService", "Power-off alarm cancel is not supported on this device.");
            return;
        }
        ...
        try {
            ...
            Log.e("AlarmService", "Failed to cancel power off alarm broadcast: " + e);
        }
    }
    ...
    private static class AlarmManagerStateChangeScheduler implements StateChangeScheduler {
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time, AlarmInstance instance, int newState) {
            ...
            Log.i("AlarmService", "Scheduling state change %d to instance %d at %s (%d)");
            ...
            // We now use AlarmReceiver (BroadcastReceiver) to start the service, so we MUST use getBroadcast.
            // Using getService/getForegroundService with a BroadcastReceiver intent is invalid.
            ...
            boolean isFiring = newState == AlarmInstance.FIRED_STATE;
            if (isFiring && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                FileLog.i("AlarmStateManager", "Scheduling AlarmClock for instance " + instance.mId + " at " + timeInMillis);
                Intent showIntent = new Intent(context, com.feldman.clock.app.MainActivity.class);
                showIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                PendingIntent showPendingIntent = PendingIntent.getActivity(context, instance.hashCode(), showIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
                
                AlarmManager.AlarmClockInfo info = new AlarmManager.AlarmClockInfo(timeInMillis, showPendingIntent);
                am.setAlarmClock(info, pendingIntent);
                return;
            }

            // Fix: Check for exact alarm permission on Android 12+ to avoid crashes
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                if (!am.canScheduleExactAlarms()) {
                    Log.w("AlarmService", "Cannot schedule exact alarm - permission denied. Falling back to setAndAllowWhileIdle");
                    FileLog.w("AlarmStateManager", "Permission denied for exact alarm. Falling back to inexact for instance " + instance.mId);
            ...
            try {
                ...
                FileLog.i("AlarmStateManager", "Scheduling ExactAndAllowWhileIdle for instance " + instance.mId + " at " + timeInMillis);
                am.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntent);
            } catch (SecurityException e) {
                Log.e("AlarmService", "Failed to schedule exact alarm: " + e.getMessage());
                FileLog.e("AlarmStateManager", "SecurityException scheduling exact alarm", e);
                // Fallback attempt
                am.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, timeInMillis, pendingIntent);
            }
        }
        ...
        public void cancelScheduledInstanceStateChange(Context context, AlarmInstance instance) {
            Log.v("AlarmService", "Canceling instance " + instance.mId + " timers");
